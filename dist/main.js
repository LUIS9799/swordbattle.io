...
this.callback({win: true, data:data})
    ...
    create() {
        ...
        this.lvlBar = new _HealthBar_js__WEBPACK_IMPORTED_MODULE_0__["default"](this, 0, this.canvas.width - (this.canvas.width/ 20), this.canvas.width - (this.canvas.width / 40), 100)
        this.lvlBar.draw()
...
this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.SPACE)
  ...
  this.nameBox = this.add.dom(window.innerWidth/2, window.innerHeight/1.7 ).createFromCache("title");
     if(this.showPromo) {
  ...
  this.done = false
  this.text = this.add.text(window.innerWidth/2, 0, 'Swordbattle.io', {
    fontSize: '64px',
    fill: '#000000'
  }).setOrigin(0.5);
  const go = () => {
